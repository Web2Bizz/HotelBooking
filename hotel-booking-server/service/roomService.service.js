...
class RoomServiceService {
  ...
  async getStatisticServiceRoom() {
    const response = await client.query(
      `SELECT 
      ROUND((SELECT COUNT(*) FROM room WHERE id_room_service_status = '0623770d-2d02-495c-a60c-f757e29bd226') * 100.0 / COUNT(*)) AS dirty_percentage,
  ...
  async editStatusServiceRoom(id_room, id_room_service_status) {
    const response = await client.query(
      `update public.room set id_room_service_status = $1 where id_room = $2`,
      [id_room_service_status, id_room]
    );
    return response;
  }
}
