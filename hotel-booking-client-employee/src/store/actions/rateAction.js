...
export const rateCreateRoomAction = (id_room_type, id_cancellation_policy, rate) => async (dispatch) => {
	try {
		dispatch(rateCreateRoom())
		await RateService.createRateRoom(id_room_type, id_cancellation_policy, rate)
		dispatch(rateCreateRoomSuccess('Расценка номеров успешно создана'))
	} catch (e) {
		dispatch(rateCreateRoomError(e.response?.data.message))
	}
}

export const rateCreateDealAction = (id_room_type, id_cancellation_policy, id_deal, rate) => async (dispatch) => {
	try {
...
export const rateEditAction = (id_room_type, id_cancellation_policy, rate, id_rate) => async (dispatch) => {
	try {
		dispatch(rateEdit())
