...
import { resetMessagesAction, rateDeleteAction, rateGetAction } from '../../store/actions/rateAction'
			...
			rate.map((item, key) => {
				if (item.id_deal === null) {
					tempData.push({
						...
						rate_to_discount: item?.discount !== null ? item?.rate - (item?.rate * item?.discount) / 100 : '-',
	...
	const columns = [
		...
		},
		{
			...
			render: (_, { cancellation_policy }) => (
				<Tag color={getColorTag(cancellation_policy, 'cancellation_policy', cancelPolicy)}>{cancellation_policy}</Tag>
			)
		},
		{
			title: 'Цена по акции',
					...
					render: (_, record) => (
						<Space size='large'>
							<Dropdown menu={{ items, onClick, record }} trigger={['click']}>
								<Space>
									<Button onClick={() => setSelectedRow(record.id_rate)} shape='circle' icon={<MoreOutlined />} />
								</Space>
							</Dropdown>
						</Space>
					)
