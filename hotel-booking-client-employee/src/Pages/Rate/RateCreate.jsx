...
const RateCreate = (props) => {
	...
	const dealData = () => {
		...
		if (dataField.typeRoomf !== null) {
			deal.map((item, key) => {
				...
				if (dataField.typeRoomf === item.id_room_type && exception.find((e) => e === item.deal_name) === undefined) {
					temp.push({
						value: item.id_deal,
	...
	const onCreateRate = () => {
		rateEditAction()
		if (props.onEditRate) {
			if (isEmpty(dataField.typeRoomf) || isEmpty(dataField.cancelPolicyf) || isEmpty(dataField.rateRoomf)) {
				errorEmptyField()
			} else {
				dispatch(
					rateEditAction(dataField.idRoomTypef, dataField.idCancellationPolicyf, dataField.rateRoomf, props.editRow)
				)
			}
		} else {
			if (isCreateDiscount) {
				...
				if (isEmpty(dataField.typeRoomf) || isEmpty(dataField.cancelPolicyf) || isEmpty(dataField.rateRoomf)) {
					...
					dispatch(rateCreateRoomAction(dataField.typeRoomf, dataField.cancelPolicyf, dataField.rateRoomf))
						...
						{isCreateRoom ? (
							...
							<Col span={4.5}>
								<Card title='Выбор акции' bordered={true}>
									<Select
										disabled={dataField.typeRoomf === null || dataField.typeRoomf === undefined ? true : false}
