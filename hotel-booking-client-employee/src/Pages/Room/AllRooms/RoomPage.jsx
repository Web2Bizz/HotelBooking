...
const Room = () => {
	...
	const onClick = ({ key }) => {
		...
		if (key === 'unblocked') {
			showConfirm('Вы точно хотите разблокировать номер?')
		}
	}
	...
	const showConfirm = (title) => {
		confirm({
			...
			cancelText: 'Нет',
			onOk() {
				dispatch(roomEditStatusAction(selectedRow))
			}
		})
	}
